...
// const API_BASE_URL = 'http://127.0.0.1:5001/soldout-99b4c/us-central1/api'; // Replace with your actual Firebase Functions URL
...
export const removeCooldown = async (auctionId) => {
  try {
    ...
    console.error('Error removing cooldown:', error.response || error.message);
    throw error;
  }
};
...
export const removeWinner = async (auctionId) => {
  try {
    const response = await axios.post(`${API_BASE_URL}/remove-winner`, { auctionId });
    return response.data;
  } catch (error) {
    console.error('Error removing winner:', error.response || error.message);
    throw error;
  }
};